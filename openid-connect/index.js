...
var defaults = {
        ...
        models: {
                ...
                client: {
                    ...
                    policies: 'loggedIn',
                    attributes: {
                        required_sig: {type: 'string', defaultsTo: 'HS256'},
...
OpenIDConnect.prototype.errorHandle = function(res, uri, error, desc) {
    if(uri) {
        ...
        //throw {type: 'error', error: error, msg: desc};
                ...
                Q(req.parsedParams).then(function(params) {
                    ...
                    req.model.client.findOne({key: params.client_id}, function(err, model) {
                        if(err || !model || model === '') {
                            ...
                            req.session.client_key = model.key;
                    ...
                    if(params.response_type == 'none') {
                        ...
                        Q.allSettled(promises).then(function(results) {
                            ...
                            if(resp.access_token && resp.id_token) {
                                ...
                                if(!req.session.client_key){
                                    console.log(req.session)
                                    throw new Error('No client key in session')
                                }
 ...
 * returns a function to be placed as middleware in connect/express routing methods. For example:
 *
 * app.get('/key', oidc.proxyKey(),  sendKey, errorHandler);
 *
 * This get the public key associated with the IdP Proxy RP
 *
 *
 */

OpenIDConnect.prototype.proxyKey = function() {
    var self = this;
